...
>>>>>>> 042e4775ce28a14e125775a7922c8eba12854029
  ...
  function handleUpload(){
    ...
    reader.onload=function(e){
      try{
        ...
        // Parse a sample to count columns
        var cols=lines[0].split(',').length;
        setSuccess('Dataset loaded successfully! '+rows.toLocaleString()+' rows, '+cols+' columns detected.');
        ...
        setError('Could not read CSV. Make sure it is a valid CSV file.');
      }
      setLoading(false);
    };
    reader.onerror=function(){setError('Failed to read file.');setLoading(false);};
...
>>>>>>> 042e4775ce28a14e125775a7922c8eba12854029
  ...
  return(
    <div style={pg}>
      ...
      <div style={{background:'rgba(14,165,233,0.07)',border:'1px solid rgba(14,165,233,0.2)',borderRadius:'12px',padding:'14px 18px',marginBottom:'24px',fontSize:'13px',color:'#38bdf8'}}>
        <strong>No backend required!</strong> Your CSV is read directly in the browser — fast and private.
      </div>
        ...
        <div
          ...
          {file?(
            <div>
              ...
              <div style={{fontSize:'12px',color:'rgba(255,255,255,0.35)'}}>{(file.size/1024/1024).toFixed(2)} MB · Ready to load</div>
            </div>
          ):(
            <div>
      ...
      <div style={{display:'flex',gap:'12px',alignItems:'center'}}>
        <button style={btn} onClick={handleUpload} disabled={loading||!file}>
          {loading?'Reading file...':'Load Dataset'}
        </button>
      ...
      <div style={{marginTop:'40px',...card}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
          {[['Credit Card CSV','V1, V2, ... V28, Amount, Time, Class (optional)','#a78bfa'],
            ['PaySim CSV','step, type, amount, nameOrig, oldbalanceOrg, newbalanceOrig, nameDest, oldbalanceDest, newbalanceDest, isFraud (optional)','#0ea5e9']].map(function(f){
