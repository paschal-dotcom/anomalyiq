...
export default function Login({ onLogin }) {
  ...
  function handle(e) {
    ...
    axios.post(BASE + url, body, { headers: HEADERS })
      .then(function(res) {
        var d = res.data;
      ...
      .catch(function(err) {
        setError(err.response?.data?.detail || 'Login failed. Make sure backend is running.');
      })
  ...
  var s = {
    ...
    card: { background: 'rgba(255,255,255,0.04)', backdropFilter: 'blur(24px)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '28px', padding: '44px 40px', width: '100%' },
    ...
    btn: { width: '100%', padding: '15px', borderRadius: '14px', border: 'none', cursor: 'pointer', fontSize: '15px', fontWeight: 800, background: 'linear-gradient(135deg,#0ea5e9,#8b5cf6)', color: 'white' },
    err: { background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: '12px', padding: '12px 16px', marginBottom: '18px', color: '#fca5a5', fontSize: '13px' },
    cred: { background: 'linear-gradient(135deg,rgba(14,165,233,0.08),rgba(139,92,246,0.08))', border: '1px solid rgba(14,165,233,0.18)', borderRadius: '12px', padding: '14px 16px', marginBottom: '22px' },
  };

  function tabBtn(t) {
    return { flex: 1, padding: '14px', background: 'none', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: 700, color: tab === t ? 'white' : 'rgba(255,255,255,0.3)', borderBottom: tab === t ? '2px solid #0ea5e9' : '2px solid transparent' };
  }

  return (
    <div style={s.page}>
      <div style={s.b1}></div><div style={s.b2}></div>
      <div style={s.left}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '44px' }}>
          <div style={{ width: '54px', height: '54px', borderRadius: '16px', background: 'linear-gradient(135deg,#0ea5e9,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '26px', fontWeight: 900, color: 'white' }}>A</div>
          <div>
            <div style={{ fontSize: '30px', fontWeight: 900, color: 'white', letterSpacing: '-1px' }}>AnomalyIQ</div>
            <div style={{ fontSize: '13px', color: 'rgba(14,165,233,0.9)', fontWeight: 700 }}>v2.0 Three-Stage Hybrid</div>
          </div>
        </div>
        ...
        <h1 style={{ fontSize: '46px', fontWeight: 900, color: 'white', lineHeight: 1.08, margin: '0 0 22px', letterSpacing: '-1.5px' }}>
          Catch Financial<br />
          <span style={{ background: 'linear-gradient(135deg,#0ea5e9,#14b8a6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Fraud</span> Before<br />
          It Happens
        </h1>
        <p style={{ color: 'rgba(255,255,255,0.48)', fontSize: '16px', lineHeight: 1.75, margin: '0 0 44px', maxWidth: '400px' }}>Three-stage hybrid AI achieving 99.80% precision on financial fraud detection.</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px', maxWidth: '460px' }}>
          {[['99.80%', 'Precision'], ['99.80%', 'Recall'], ['99.80%', 'F1-Score'], ['100%', 'AUC-ROC']].map(function(m) {
            return (<div key={m[1]} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '14px', padding: '16px 10px', textAlign: 'center' }}>
              <div style={{ fontSize: '20px', fontWeight: 900, background: 'linear-gradient(135deg,#0ea5e9,#8b5cf6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{m[0]}</div>
        ...
        <div style={{ marginTop: '48px', paddingTop: '32px', borderTop: '1px solid rgba(255,255,255,0.08)', fontSize: '13px', color: 'rgba(255,255,255,0.35)' }}>
          Godfrey Okoye University — Dept. of Computer Science
        </div>
      </div>
      <div style={s.right}>
        <div style={{ width: '100%' }}>
          <div style={s.card}>
            ...
            <div style={{ color: 'white', fontSize: '22px', fontWeight: 900, margin: '0 0 8px', textAlign: 'center' }}>{tab === 'login' ? 'Welcome back' : 'Join AnomalyIQ'}</div>
            <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: '13px', margin: '0 0 28px', textAlign: 'center' }}>{tab === 'login' ? 'Sign in to your personal dashboard' : 'Create your analyst account'}</div>
            <form onSubmit={handle}>
              ...
              {tab === 'login' && <div style={s.cred}><div style={{ fontSize: '10px', fontWeight: 800, color: '#38bdf8', letterSpacing: '1px', marginBottom: '5px' }}>DEFAULT ADMIN CREDENTIALS</div><div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.55)' }}>admin@anomalyiq.com / Admin@1234</div></div>}
