    ...
    <aside className="fixed left-0 top-0 h-screen w-64 bg-white border-r border-slate-200 flex flex-col z-50">
      <div className="px-6 py-5 border-b border-slate-100">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center text-white text-lg font-bold">A</div>
          <div><div className="font-bold text-slate-800 text-lg">AnomalyIQ</div><div className="text-xs text-slate-400">v2.0 Three-Stage Hybrid</div></div>
        </div>
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto">
        ...
        {links.map(function(item){var Icon=item.icon; return(
          <NavLink key={item.to} to={item.to} end={item.to==='/'}
            className={function(p){return "flex items-center gap-3 px-3 py-2.5 rounded-xl mb-0.5 text-sm font-medium "+(p.isActive?'bg-teal-50 text-teal-700 border border-teal-100':'text-slate-600 hover:bg-slate-50');}}>
        ...
        <div className="mt-6 border-t border-slate-100 pt-4">
          ...
          {steps.map(function(s){var done=!!pipelineStatus[s.key]; return(
            <div key={s.key} className="flex items-center gap-2.5 px-3 py-1.5">
              <div className={"w-2 h-2 rounded-full "+(done?'bg-teal-400':'bg-slate-200')}></div>
