...
export default function Upload({onDatasetLoaded}){
  ...
  function handleUpload(){
    ...
    axios.post(BASE+'/api/upload',fd,{
      headers:{'Content-Type':'multipart/form-data','Authorization':'Bearer '+token},
      timeout:300000
    })
    .then(function(res){
      var data=Object.assign({},res.data,{dataset_type:dtype,filename:file.name});
      setSuccess('Dataset uploaded! '+( res.data.rows||'')+' rows loaded. Go to Run Detection to start.');
      localStorage.setItem('anomalyiq_dataset',JSON.stringify(data));
      onDatasetLoaded&&onDatasetLoaded(data);
    })
    .catch(function(err){
    ...
    <div style={pg}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px',marginBottom:'24px'}}>
        {[['creditcard','Credit Card (MLG-ULB)','EU','European cardholders · 284,807 transactions · 30 PCA features','#a78bfa'],
          ...
          return(<div key={d[0]} onClick={function(){setDtype(d[0]);}} style={{background:active?'rgba(14,165,233,0.15)':'rgba(255,255,255,0.03)',border:active?'1px solid rgba(14,165,233,0.35)':'1px solid rgba(255,255,255,0.07)',borderRadius:'14px',padding:'20px',cursor:'pointer',transition:'all 0.2s'}}>
      ...
      <div style={{...card,marginBottom:'24px'}}>
        ...
        <div onDragOver={function(e){e.preventDefault();setDragging(true);}}
          ...
          {file
            ?<div><div style={{fontSize:'15px',fontWeight:700,color:'#38bdf8',marginBottom:'4px'}}>{file.name}</div><div style={{fontSize:'12px',color:'rgba(255,255,255,0.35)'}}>{(file.size/1024/1024).toFixed(2)} MB · Ready to upload</div></div>
            :<div><div style={{fontSize:'15px',color:'rgba(255,255,255,0.6)',marginBottom:'6px'}}>Drop your CSV here or click to browse</div><div style={{fontSize:'12px',color:'rgba(255,255,255,0.3)'}}>Supports .csv files only</div></div>
          }
        </div>
      </div>
      ...
      <div style={{display:'flex',gap:'12px',alignItems:'center'}}>
        <button style={{...btn,opacity:loading||!file?0.5:1,cursor:loading||!file?'not-allowed':'pointer'}} onClick={handleUpload} disabled={loading||!file}>
          {loading?'Uploading...':'Upload Dataset'}
        </button>
        {file&&!loading&&<button onClick={function(){setFile(null);setError('');setSuccess('');}} style={{padding:'13px 20px',borderRadius:'12px',border:'1px solid rgba(255,255,255,0.12)',background:'transparent',color:'rgba(255,255,255,0.5)',fontSize:'14px',cursor:'pointer'}}>Clear</button>}
      </div>

      {loading&&<div style={{marginTop:'12px',fontSize:'12px',color:'rgba(255,255,255,0.35)'}}>Uploading to local backend... this may take a moment for large files.</div>}
    </div>
  );
}