...
const STYLES = `
  ...
  .login-hero {
    flex: 1;
    background: linear-gradient(145deg, #0d1117 0%, #0e2240 45%, #0c3d5a 100%);
    display: flex;
  ...
  @media (max-width: 1024px) {
    ...
    .login-form-panel { width: 420px; padding: 48px 36px; }
  }
  ...
  @media (max-width: 480px) {
    .login-form-panel { padding: 32px 16px; padding-top: 48px; }
...
function injectStyles() {
  ...
  tag.id = 'login-responsive-styles';
...
function RegisterForm({ onSuccess, onSwitch }) {
  ...
  const handleRegister = async (e) => {
    ...
    if (!/^[a-zA-Z0-9_]+$/.test(form.username)) { setError('Username: letters, numbers and underscores only.'); return; }
    ...
    <>
      ...
      <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
        ...
        <DarkField label="FULL NAME">
          <input
            type="text" placeholder="e.g. Paschal Nwagor"
        ...
        {/* USERNAME + EMAIL — stacks to 1 col on small mobile */}
        <div className="login-form-grid">
          <DarkField label="USERNAME">
            <input
              type="text" placeholder="e.g. paschal"
        ...
        {/* PASSWORD + CONFIRM — stacks to 1 col on small mobile */}
        <div className="login-form-grid">
          <DarkField label="PASSWORD">
            <div style={{ position: 'relative' }}>
              <input
